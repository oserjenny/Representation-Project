...
Voting vars recode
JO note: reference prior code ("count all 2010 vars..."") to create more parsimonious code; for now, do one at a time.

Validated vote variables, general election:
2010, 2012, 2014
...
2010 recode as factor
```{r}

tbl_panel <- tbl_panel %>% 
  mutate(VV_general_10_fac = as_factor(VV_general_10)) 

# check successful mutate
tbl_panel %>% 
  count(VV_general_10, VV_general_10_fac)
```


HELP: 
2010 recode as binary
JO trying to adapt ICCS code - but this code was for the factor version of variable, but seems cleaner code to use the original numeric categories.
```{r}

tbl_panel <- tbl_panel %>% 
  mutate(VV_general_10_bin = fct_collapse(VV_general_10,
  "vote"     = c(1, 2, 3, 4, 5),
  "not vote" = c(6)
  "NA"       = c(9)
...
2012 recode as factor
```{r}

tbl_panel <- tbl_panel %>% 
  mutate(VV_general_12_fac = as_factor(VV_general_12)) 

# check successful mutate
tbl_panel %>% 
  count(VV_general_12, VV_general_12_fac)
```
...
2014 recode as factor
```{r}

tbl_panel <- tbl_panel %>% 
  mutate(VV_general_14_fac = as_factor(VV_general_14)) 

# check successful mutate
tbl_panel %>% 
  count(VV_general_14, VV_general_14_fac)
```


NOW CODING POL PARTIC IN ADDITION TO VOTING:
...
tbl_panel <- tbl_panel %>% 
  mutate(CC10_417a_1_fac = as_factor(CC10_417a_1)) 

# check successful mutate
tbl_panel %>% 
  count(CC10_417a_1, CC10_417a_1_fac)
```
HELP: JO question, does the output mean there are no missing values here? Or are they just not displayed? Same question for all vars in this series.
...
tbl_panel <- tbl_panel %>% 
  mutate(CC12_417a_1_fac = as_factor(CC12_417a_1)) 
...
HELP REQUESTS TO COMPLETE THIS CODING: 
**Complete recoding of "attend meeting"variables for all three years, and display as figures (bar chart or line chart, whatever you think displays best) similar to "vote" above:
CC10_417a_1
CC12_417a_1
CC14_417a_1

**Do similar var recodes and figure display
for: 
